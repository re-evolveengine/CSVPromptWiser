...
def test_process_one_chunk_success(mock_client, mock_chunk_manager, sample_dataframe):
    mock_chunk_manager.get_next_chunk.return_value = (sample_dataframe, "chunk123")
...
def test_no_more_chunks(mock_client, mock_chunk_manager):
...
def test_fatal_error(mock_client, mock_chunk_manager, sample_dataframe):
    ...
    with patch(runner_path) as mock_runner_cls:
        ...
        processor = ChunkProcessor("prompt", mock_client, mock_chunk_manager)
        result = processor.process_next_chunk()
...
def test_retryable_error(mock_client, mock_chunk_manager, sample_dataframe):
    mock_chunk_manager.get_next_chunk.return_value = (sample_dataframe, "chunkX")
...
def test_unexpected_error(mock_client, mock_chunk_manager, sample_dataframe):
